...
def womanual():
    data = []
    example1 = "Yo, you busy tomorrow? Yes, sorry I have plans already. OK, no problem. How about Tuesday? I have bootcamp on Tuesday. Alright then. "
    data.append([example1,[{'text': text, 'answer_start': example1.index(text)} for text in ("","","","")]])
    example1 = "Hey! You free on Thursday? No, sorry.. How about Friday? I can't on Friday either. Oh ok, that's fine. "
    data.append([example1,[{'text': text, 'answer_start': example1.index(text)} for text in ("","","","")]])
    example1 = "Sup! Want to go bowling? When? Sunday? Sure! Wait, never mind. I won't be here on Sunday. Is Saturday ok? Yes, that's fine. Great, excited! "
    data.append([example1,[{'text': text, 'answer_start': example1.index(text)} for text in ("go bowling","Saturday","","")]])
    return data
...
if __name__ == '__main__':
    ...
    outpath = "./data/new_data_test.json"
    # outpath = sys.argv[1]
