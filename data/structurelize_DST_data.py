...
yes = ['Sure. ', 'Ok. ', 'Sure! ', 'OK! ', 'Great. ', 'Sounds good. ', 'Of course', 'Good', 'Yes', 'Nice', 'Yeah...']
...
cu = [" there"," then"]
refuse = ["Sorry, I", "No..., I", 'No. ', 'Sorry. ', 'Nah ']
...
no_similiar = 'nope/not/reject/no/no way/of course not/'.split('/') 
yes_similiar = 'exactly/That\'s right/True dat/OK/agree/yeah/can\'t wait/Yep/Yup/okay/k/alright/of course'.split('/') 
...
def Gen_Classfier_Data(rows):
    ...
    for _ in range(rows):
        ...
        get_no_similiar = random.choice(no_similiar)
        get_yes_similiar = random.choice(yes_similiar)
      ...
      #[0, 0, 0, 0, 1]
        train_data_classifier.append([get_refuse + get_tno, [0, 1, 0, 0, 1]])
        train_data_YN_classifier.append([get_refuse + get_tno, 0])
        train_data_QA.append([get_refuse + get_tno, Question_dict['Logic'], 'No'])

        train_data_classifier.append([get_refuse + get_dno, [0, 1, 0, 0, 1]])
        train_data_YN_classifier.append([get_refuse + get_dno, 0])
        train_data_QA.append([get_refuse + get_dno, Question_dict['Logic'], 'No'])
        
      #[0, 0, 0, 0, 1]
        train_data_classifier.append([get_yes_similiar, [0, 0, 0, 0, 1]])
        train_data_YN_classifier.append([get_yes_similiar, 1])
        train_data_QA.append([get_yes_similiar, Question_dict['Logic'], 'Yes'])

      #[0, 0, 0, 0, 1]
        train_data_classifier.append([get_no_similiar + get_tno, [0, 1, 0, 0, 1]])
        train_data_YN_classifier.append([get_no_similiar + get_tno, 0])
        train_data_QA.append([get_no_similiar, Question_dict['Logic'], 'No'])
        
        train_data_classifier.append([get_refuse, [0, 0, 0, 0, 1]])
        train_data_YN_classifier.append([get_refuse, 0])
        train_data_QA.append([get_refuse + get_dno, Question_dict['Logic'], 'No'])

        train_data_classifier.append([get_refuse + get_dno + random.choice([get_activity, get_day]), [0, 1, 0, 0, 1]])
        train_data_YN_classifier.append([get_refuse + get_dno + random.choice([get_activity, get_day]), 0])
        train_data_QA.append([get_refuse + get_dno, Question_dict['Logic'], 'No'])
    ...
    print(row_end_for_Domain_training, row_end_for_YN_training)

    # 80% training data, 20% test data
    # Domain lassifier
    #Domain_classifier.iloc[:row_end_for_Domain_training].to_csv('struture_Domain_classifier_train.csv', index=False)
    #Domain_classifier.iloc[row_end_for_Domain_training:].to_csv('struture_Domain_classifier_test.csv', index=False)
    # Yes No 
    YN_classifier.iloc[:row_end_for_YN_training].to_csv('struture_YN_classifier_train.csv', index=False)
    YN_classifier.iloc[row_end_for_YN_training:].to_csv('struture_YN_classifier_test.csv', index=False)
